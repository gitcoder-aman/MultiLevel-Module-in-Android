...
class MyAppHttpClientBuilder(
    ...
    fun build() : HttpClient{
        return HttpClient(CIO){
            expectSuccess = true  //if getting error in apiClient then throw error
            ...
            defaultRequest {
                url {
                    protocol = this@MyAppHttpClientBuilder.protocol
                    host = this@MyAppHttpClientBuilder.host
                    this@MyAppHttpClientBuilder.port?.let { port = it }
                }
...
//val builder = MyAppHttpClientBuilder().port(8080).host("").protocol(URLProtocol.HTTP).build()